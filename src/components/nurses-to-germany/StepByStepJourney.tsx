    ...
    return (
        <section className="relative w-full overflow-hidden py-16">
            ...
            <div className="absolute bottom-0 left-0 right-0 z-0">
                <svg xmlns="http://www.w3.org/2000/svg" width="1440" height="373" viewBox="0 0 1440 373" fill="none" className="w-full">
            ...
            {/* Title */}
            <div className="relative z-10 mx-auto mb-12 max-w-[894px] px-4">
                <h2 className="text-center font-['Albert_Sans',sans-serif] text-[44px] font-medium leading-[1.29] tracking-[-0.88px] text-[#18191f]">
                    <span>Our Step-by-Step </span>
                    <span className="text-[#0829e6]">Journey</span>
                </h2>
            </div>

            {/* Steps Grid */}
            <div className="relative z-10 mx-auto grid max-w-[894px] grid-cols-2 gap-8 px-4">
                {/* Left Column */}
                <div className="flex flex-col gap-7">
                    {steps.slice(0, 4).map((step) => (
                        <div key={step.number} className="flex items-center">
                            {/* Number Circle */}
                            <div className="relative z-10 mr-[-28px] flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-solid border-[#5E72E4] bg-white p-1">
                                <p className="flex h-full w-full items-center justify-center rounded-full bg-[#5e72e4] font-['Albert_Sans',sans-serif] text-base font-medium text-white">
                                    {step.number}
                                </p>
                            </div>
                            {/* Text Box */}
                            <div className="flex h-14 flex-1 items-center rounded-r-2xl bg-[#5e72e4] py-2.5 pl-9 pr-3">
                                <p className="font-['Albert_Sans',sans-serif] text-base font-medium leading-[1.29] tracking-[-0.32px] text-white">
                                    {step.text}
                                </p>
                            </div>
                        </div>
